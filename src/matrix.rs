...
impl<'a> Matrix<'a> {
    ...
    fn schedule_nightly_reboot(self: &mut Self, reboot_time: u8){
        ...
        let tomorrow_3am = tomorrow.with_hour(reboot_time.into()).unwrap().with_minute(0).unwrap();
        let time_until_tomorrow_3am = tomorrow_3am - now;
        info!("Setting up delayed command to reboot the scoreboard at 3AM tomorrow, in {:?}", time_until_tomorrow_3am);
        self.scheduler_sender
            .send(scheduler::DelayedCommand::new(
                scheduler::Command::MatrixCommand(common::MatrixCommand::Reboot()),
                Some(time_until_tomorrow_3am.to_std().unwrap()),
            ))
            .unwrap();
